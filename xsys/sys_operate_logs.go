...
func (o *SysOperateLogs) Get(tx *sql.Tx, ctx *handle.Context) (interface{}, error) {
	...
	query := `
		SELECT sys_operate_log.id, sys_operate_log.user_name_, sys_operate_log.method_, 
			sys_menu.name_ AS menu_name_, sys_operate_log.path_, sys_operate_log.params_, 
			sys_operate_log.values_, sys_operate_log.start_, sys_operate_log.end_, sys_operate_log.duration_, 
		...
		FROM sys_operate_log, sys_menu
        WHERE sys_operate_log.menu_id_ = sys_menu.id
	`
